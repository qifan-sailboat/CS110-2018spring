...
int sum_vectorized( int n, int *a )
{
    ...
    int final = A[0] + A[1] + A[2] + A[3];

    for (int i = n / 4 * 4; i < n; i++) final += a[i];
...
int sum_vectorized_unrolled( int n, int *a )
{
    ...
    for (int i = 0; i < n / 16 * 16; i += 16){
        __m128i t1 = _mm_loadu_si128((__m128i *) (a + i));
        sum = _mm_add_epi32(t1, sum);
        __m128i t2 = _mm_loadu_si128((__m128i *) (a + i + 4));
        sum = _mm_add_epi32(t2, sum);
        __m128i t3 = _mm_loadu_si128((__m128i *) (a + i + 8));
        sum = _mm_add_epi32(t3, sum);
        __m128i t4 = _mm_loadu_si128((__m128i *) (a + i + 12));
        sum = _mm_add_epi32(t4, sum);
    }
    ...
    for (int i = n / 16 * 16; i < n; i++) final += a[i];
