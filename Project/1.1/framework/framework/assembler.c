...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
  ...
  while (fgets(buf, BUF_SIZE, input) != NULL) {
    ...
    skip_comments(buf);
    /* Check whether this is line has comments */
    /* and skip them */
    line += 1;
    /* Add one more line */
    temp = strtok(buf, IGNORE_CHARS);
    /* get the first valid part */

    if (temp == NULL) continue;
    /* if there is no new element, then go the the next line. */
    /*printf("%s\n", temp);*/
    result = add_if_label(line, temp, addr, symtbl);
    /*printf("%d\n", result);*/
    /* Check whether it is a label*/
    /* If so, remark and skip */
    /*fprintf(output, "%d\n", result);*/

    if (result == -1){
      /* invalid label or existed*/
      isErr = 1;
      /* record the error */
      ...
      /* get the next part */
      if (temp == NULL) continue;
      /* if there is nothing in this line, */
      /* then go to the next line */
      /* judge whether the second part is a label */
    ...
    /* store parameters */
    temp = strtok(NULL, IGNORE_CHARS);
    /* get the first element for the instruction */
    while (temp != NULL) {
      /* while the line is still having elements */
      args[num_args] = temp;
      /* store the args */
      if (add_if_label(line, temp, addr, symtbl) == 1) {
        /* judge whether it is a label */
        isErr = 1;
        /* There is an additional lable, therefore it is invalid */
        continue;
      }
      if (num_args >= MAX_ARGS){
        /* Beyond the largest args num. */
        isErr = 1;
        num_args += 1;
        /* This is an error */
        /* we still need to add 1 to num_args to get the correct num_arg*/
        /* which will help us in final print out */
        /* it has extra argumented elements, therefore do not print out */
        raise_extra_argument_error(line, temp);
    ...
    if (num_args <= 3) {
    /* for all instructions, 3 args is the largest number of args */
    /* therefore for those who has more than 4 args, */
    /* it is bount to be an illegal instruction. */
      lines_real = write_pass_one(output, instr, args, num_args);
    /* in case there are pseudoinstructions */
      addr = addr + 4 * lines_real;
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    while (fgets(buf, BUF_SIZE, input) != NULL){
      char* temp = strtok(buf, IGNORE_CHARS);
      char instr[5];
      /* varibles */
      ...
      if (translate_inst(output, instr, args, num_args, addr, symtbl, reltbl) == -1){
        /* whether it is a valid instruction, judge by -1 */
        /* it has -1 */
        /* error */
        isErr = 1;
        raise_instruction_error(line, instr, args, num_args);
      }else addr += 4;
      /* add the address for 4 bit */
