...
write_machine_code_find_text:	
	lw $a0, 24($sp)
	jal readline
	blt $v0, $zero, write_machine_code_error	# Invalid line reached
	move $a0, $v1
	la $a1, textLabel
	jal streq
	bne $v0, $zero, write_machine_code_find_text
	...
	addi 				$k1, $zero, 0
	# set the original byte offset as 0
...
write_machine_code_next_inst:
	...
	move 				$a0, $v1
	# save what we have read in the first argument
	addi 				$a1, $zero, 16
	# set the second argument as 16 to represent its base
	jal 				parse_int
	# convert the instr as a num
	move 				$s7, $v0
	# save the int we have converted
	...
	move 				$a0, $s7
	# load the int to the first Argument
	jal 				inst_needs_relocation
	# get the instruction code
	beq 				$v0, $zero, write_machine_code_to_file
	# judge whether it is a legal instruction
	...
	move 				$a0, $s7
	# load the instr int to the first arg
	move 				$a1, $k1
	# load the bit offset to the second arg
	lw 					$a2, 20($sp)
	# load the $a2 to the third arg
	lw 					$a3, 16($sp)
	# load the $a3 to the third arg
	jal 				relocate_inst
	# relocate the instruction
	move 				$s7, $v0
	# save the final int to write
...
write_machine_code_to_file:
	...
	#a0 already has the intstruction
	move 				$a0, $s7
	# load the final instr int to the 1st arg
	la 					$a1, hex_buffer
	# load the buffer to the 2nd arg
	jal 				hex_to_str
	# change the hexdecimal num to str
	...
	bne $k0, $0, write_machine_code_next_inst
