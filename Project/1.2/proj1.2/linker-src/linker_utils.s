...
relocate_inst:
	...
	addi 			$sp, $sp, -4
	sw 				$ra, 0($sp)
	# first, save the addr where we cone from

	addi 			$sp, $sp, -4
	sw 				$a0, 0($sp)
	# save $a0 since $a0 will be used
	addi 			$sp, $sp, -4
	sw 				$a1, 0($sp)
	# save $a1 since $a1 will be used
	addi 			$sp, $sp, -4
	sw 				$a2, 0($sp)
	# save $a2 since $a2 will be used
	addi 			$sp, $sp, -4
	sw 				$a3, 0($sp)
	# save $a3 since $a3 will be used

	lw	 			$a0, 8($sp)
	lw	 			$a1, 0($sp)
	# store the first argument as $a1
	# which is the address we are looking for
	# stire the second argument as $a3
	# which is the list we are looking up
	jal				symbol_for_addr
	# get the name from the relocation table

	beq 			$v0, $zero, err
	# check whether the $v0 is NULL
	# which means there is no such an address in the table
	move 			$a1, $v0
	# load second argument as the ptr of the name we found
	lw 				$a0, 4($sp)
	# the first argument is the table where we want to relocate
	jal 			addr_for_symbol
	# search for the right place for the addr

	addi 			$t2, $zero, -1
	beq 			$v0, $t0, err
	# check whether we successfully add the address in the table

	srl 			$v0, $v0, 2
	# shift 2 bits right
	lw 				$t2, 12($sp)
	# 12($sp) is the original $a0
	andi			$v0, $v0, 0x03ffffff
	# 0x03ffffff = 0000 0011 1111 1111 1111 1111 1111 1111 (in binary)
	# get the third to 29st location
	lw 				$t2, 12($sp)
	andi 			$t3, $t2, 0xfc000000
	# 0xfc000000 = 1111 1100 0000 0000 0000 0000 0000 0000 (in binary)
	# get the first 6 bits of $a0
	# then we need to paste $t3 and $v0 to get the relocated instruction
	# using "or"
	or 				$v0, $t3, $v0
	# paste the address
